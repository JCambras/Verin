...
`src/contracts/decision-core/serialization.ts`, schema version 1.6.0 and serializer version
...
Bundle preimage version `decision-input-bundle/1.6.0` excludes `id` and `bundleHash`, because
identity is not a material evaluation input, and sorts the instruction-version and
evidence-snapshot reference collections. Decision preimage version `decision-record/1.6.0`
excludes only `decisionHash`; the decision ID and all order-significant traces, stages, and plan
steps remain bound. The 1.6.0 shapes tenant-scope every named configuration, evidence, subject, scope,
...
require one tenant throughout every external action and plan, tenant-scope and canonicalize role sets,
