...
- the strict `verin-real-derived-replay/1.10.0` payload: entity-kind-scoped destination, ownership,
  ...
  lifecycle state is recomputed from effectivity instants at `evaluation.asOf`. Every supported class records typed expected and observed
...
- semantic contract `verin-real-derived-semantics/1.11.0`; its declarative bytes and the complete
