...
> **Authority after ADR-0060.** This artifact describes current Verin, which F2 preserves as the
> read-only behavioral, compatibility, and evidence oracle until an explicitly authorized cutover. Its
> product truth and architecture-neutral guarantees carry on the charter's terms, but its four-layer
> composition and other implementation choices are comparison evidence, not a selected fourth-generation
> architecture. F1 authorizes only disposable experiments now; F6-F8 keep the architecture open and
> require a later captain decision before durable replacement implementation.
...
> provenance-split measurement that refuses to blend, ADR-0052). Every
...
provenance-split replay corpus (`docs/corpus.md` + `fixtures/corpus/`, ADR-0052 - synthetic substrate
