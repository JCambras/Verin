...
| `dependency-rule` (ts-morph: compiler-resolved aliases, static+relative+dynamic+direct-or-indirect-CommonJS+import-type+import-equals+source-declarations+triple-slash+implicit JSX runtime; fail-closed local paths and ES-only platform surface; Zod-only external allowlist in `contracts/`) | layer boundary (#1) | PF-002 + extensions + companions |
