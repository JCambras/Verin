...
async function main(): Promise<void> {
  ...
  process.stdout.write(
    [
      ...
      `decision chain after restore: ${afterDecisionChain.ok ? `L1-L4 + ${afterDecisionChain.replaySourcesChecked} SOURCES VERIFIED` : `BROKEN — ${afterDecisionChain.ledger.levels.find((level) => !level.ok)?.reason ?? afterDecisionChain.replaySourceReason ?? "unknown"}`}`,
