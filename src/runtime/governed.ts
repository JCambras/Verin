...
export type RequestCorrelation = { readonly [sealed]: "RequestCorrelation"; readonly kind: "RequestCorrelation"; readonly fields: { readonly requestId: { readonly value: string; readonly purpose: "requestId" } } };
...
export const mintRequestId = (): RequestId => { const r = { value: randomBytes(16).toString("hex"), purpose: "requestId" } as RequestId; MINTED.add(r); return r; };
...
const atMostOne = (r: QueryResult, schema: z.ZodType) => { if (r.rows.length > 1) throw new Error("cardinality at-most-one violated"); return r.rows.length ? schema.parse(r.rows[0]) : null; };
...
export function createGovernedRuntime(role: "web" | "tooling"): Gateway {
  ...
  delete process.env["VERIN_APP_DATABASE_URL"]; delete process.env["VERIN_COOKIE_KEY"];
  ...
  for (const r of REGISTRY) {
    const reg = REGISTRY_EFFECTS[r.id.id], adm = ADMITTED[r.id.id];
    if (r.class === "store") {
      ...
      const a = deriveEffect(reg), b = deriveEffect(adm.constructedDefinition);
  ...
  const graph: Record<string, unknown>[] = [], invocations: Record<string, unknown>[] = [], rawExecutions: Record<string, unknown>[] = [], logs: Record<string, unknown>[] = [];
  ...
  async function enter<T>(id: OpKey, c: RequestCorrelation, fn: () => Promise<T>): Promise<T> {
    ...
    if (!r.permittedParents.includes(parent ? parent.op : "entry")) throw new Error(`operation '${id}' entered under parent '${parent ? parent.op : "entry"}', which is not in its declared permittedParents`);
    ...
    let outcome = "ok";
    try { return await als.run({ node, op: id }, fn); }
    catch (e) { outcome = "error"; throw e; }
    finally {
      span.setAttributes({ requestId: String(rid.value), outcome }); span.end();
      counters.get(id)!.add(1, { requestId: String(rid.value), ...(fx ? { semanticEffectId: fx } : {}) });
      const rec = { name: opName(id), node, ...(fx ? { semanticEffectId: fx } : {}), fields: { requestId: String(rid.value), outcome, traceId: span.spanContext().traceId, spanId: span.spanContext().spanId } };
      logs.push(rec); process.stdout.write(JSON.stringify(rec) + "\n");
    }
  }
  async function runStore(id: OpKey, c: RequestCorrelation, values: StoreValues): Promise<unknown> {
    ...
    return enter(id, c, async () => {
      ...
      const def = ADMITTED[id].constructedDefinition, fx = deriveEffect(def);
  ...
  G = Object.freeze({
    ...
    openCookieValue: (v: string) => {
      ...
      const token = v.slice(0, dot), sig = Buffer.from(v.slice(dot + 1)), want = Buffer.from(hmac(token));
  ...
  SNAP = async () => {
    ...
    const spans = spanExporter.getFinishedSpans().map((s) => { const node = spanToNode.get(s.spanContext().spanId) ?? "unmapped"; const fx = fxOf(node); return { node, name: s.name, completed: true, ...(fx ? { semanticEffectId: fx } : {}), attributes: s.attributes }; });
    const metricsOut = metricExporter.getMetrics().flatMap((rm) => rm.scopeMetrics.flatMap((sm) => sm.metrics.flatMap((m) => m.dataPoints.map((dp) => ({ name: m.descriptor.name, value: dp.value as number, attributes: dp.attributes })))));
    return {
      registry: REGISTRY.map((r) => ({ id: r.id.id, class: r.class, owner: r.owner, correlation: r.correlation, metric: r.metric, attributes: r.attributes, permittedParents: r.permittedParents, gatewayEntry: r.gatewayEntry, slice: r.slice, ...(r.class === "store" ? { semanticEffect: REGISTRY_EFFECTS[r.id.id], semanticEffectId: derived.get(r.id.id)!.id } : {}) })),
      ...
      graph, invocations, rawExecutions, emissions: { spans, metrics: metricsOut, logs },
      correlationTable: Object.fromEntries(REGISTRY.map((r) => [r.id.id, r.correlation])), namingPattern: NAMING_PATTERN,
    };
  };
  (globalThis as { [SLOT]?: Slot })[SLOT] = { gateway: G, snapshot: SNAP };
  return G;
}
export function getGateway(): Gateway { const s = slot(); if (!s) throw new Error("the governed runtime is not constructed; only a composition root may construct it"); return s.gateway; }
export function snapshotEvidence(): Promise<Record<string, unknown>> { const s = slot(); if (!s) throw new Error("no runtime to snapshot"); return s.snapshot(); }
