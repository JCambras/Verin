...
export async function createApplication(
  db: SqlDb, p: Principal, input: { householdId: string; contactId: string; accountType: AccountType }, idempotencyKey?: string,
  ...
  return auditedWrite<{ id: string; idempotencyKey: string }>({
    db, orgId: p.orgId, actor: p.userId, action: "application.create", entityType: "AccountOpeningApplication", entityId: id,
    ...
    perform: async (tx) => {
      await tx.query(
        ...
        [id, p.orgId, input.householdId, input.contactId, input.accountType, finalizeKey, now],
      );
...
export async function setEsignRequested(
  db: SqlDb, p: Principal, applicationId: string, token: string, idempotencyKey?: string,
): Promise<Result<{ token: string }>> {
  return auditedWrite<{ token: string }>({
    db, orgId: p.orgId, actor: p.userId, action: "application.request-esign", entityType: "AccountOpeningApplication", entityId: applicationId,
