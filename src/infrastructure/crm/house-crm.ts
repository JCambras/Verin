 ...
 * EVERY read is org-scoped by the principal's orgId (org-id-required fence) —
 * org_id is never client-supplied. Provenance source = verin-crm on every row
 * (charter #3).
 */
...
export async function createHousehold(
  ...
  return auditedWrite<Household>({
    ...
    db, orgId: a.orgId, actor: a.actorUserId, action: "household.create", entityType: "Household", entityId: id,
    ...
    perform: async (tx) => {
      await tx.query(
        ...
        [id, a.orgId, input.name, a.actorUserId, status, createdAt, prov.source, prov.asOf, prov.confidence],
      );
      return { id, orgId: a.orgId, name: input.name, primaryContactId: null, advisorUserId: a.actorUserId, status, createdAt, provenance: prov };
    },
  });
}
...
export async function updateHouseholdName(db: SqlDb, a: WriteActor, id: string, name: string): Promise<Result<Household>> {
  ...
  return auditedWrite<Household>({
    db, orgId: a.orgId, actor: a.actorUserId, action: "household.update", entityType: "Household", entityId: id,
...
export async function createContact(
  ...
  return auditedWrite<Contact>({
    db, orgId: a.orgId, actor: a.actorUserId, action: "contact.create", entityType: "Contact", entityId: id,
    ...
    perform: async (tx) => {
      await tx.query(
        ...
        [id, a.orgId, input.householdId, input.firstName, input.lastName, input.email ?? null, input.phone ?? null, createdAt, prov.source, prov.asOf, prov.confidence],
      );
      return { id, orgId: a.orgId, householdId: input.householdId, firstName: input.firstName, lastName: input.lastName, email: input.email ?? null, phone: input.phone ?? null, createdAt, provenance: prov };
    },
  });
}
...
export async function createFinancialAccount(
  ...
  return auditedWrite<FinancialAccount>({
    db, orgId: a.orgId, actor: a.actorUserId, action: "financial_account.create", entityType: "FinancialAccount", entityId: id,
    ...
    perform: async (tx) => {
      await tx.query(
        ...
        [id, a.orgId, input.householdId, input.accountType, input.custodian ?? null, currency, status, openDate, createdAt, prov.source, prov.asOf, prov.confidence],
      );
      return { id, orgId: a.orgId, householdId: input.householdId, accountType: input.accountType, custodian: input.custodian ?? null, balanceMinorUnits: null, currency, status, openDate, createdAt, provenance: prov };
    },
  });
}
...
export async function createTask(
  ...
  return auditedWrite<Task>({
    db, orgId: a.orgId, actor: a.actorUserId, action: "task.create", entityType: "Task", entityId: id,
    ...
    perform: async (tx) => {
      await tx.query(
        ...
        [id, a.orgId, input.householdId ?? null, input.subject, createdAt, prov.source, prov.asOf, prov.confidence],
      );
      return { id, orgId: a.orgId, householdId: input.householdId ?? null, subject: input.subject, status: "not-started", dueDate: null, assigneeUserId: null, createdAt, provenance: prov };
    },
  });
}
