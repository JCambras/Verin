 ...
 * compile - and every one of those is operator-recoverable by cause (D-257). Held
 ...
 * derive the whole class from the modules that own it (D-260).
 */
...
export function configuredFlow(): Result<CompiledFlow, AppError> {
  ...
  // operator-recoverable cause as one that fails to load or compile (D-257):
