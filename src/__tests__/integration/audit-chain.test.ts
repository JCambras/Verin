...
import { verifyAndListOrgChain, verifyOrgChain, drainOutbox, discardedAuditEventWork } from "@infra/audit/audit-store";
...
describe("tamper-evident audit chain (integration)", () => {
  ...
  it("an audited write produces a verifiable chain entry with the real actor", async () => {
    ...
    const chain = (await verifyAndListOrgChain(db, AUDIT_GRANT)).rows;
  ...
  it("reclaims a stale 'claimed' outbox row (a crash between claim and delete must not lose the entry)", async () => {
    ...
    expect((await verifyAndListOrgChain(db, AUDIT_GRANT)).rows.length).toBe(1);
    expect((await verifyOrgChain(db, TENANT)).ok).toBe(true);
  ...
  it("a poison outbox row parks at the attempts cap and is excluded from later drains (dead-letter, no silent churn)", async () => {
    ...
    expect((await verifyAndListOrgChain(db, AUDIT_GRANT)).rows.length).toBe(0);
  });
