 ...
 *
 * It now answers in the shared refusal shape (D-257), and this asserts the two
 ...
 * the refusal, the log line and the fence all agreed it existed (D-260).
 */
...
describe("POST /api/esign/simulate-sign answers a refusal in the shared shape", () => {
  ...
  it("answers a superseded configuration version with retry-later, paced, and a quotable reference", async () => {
    ...
    // The two configuration version ids are deployment internals (D-258), and the
