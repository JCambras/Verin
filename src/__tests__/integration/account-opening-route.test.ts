...
const injected = vi.hoisted(() => ({
   ...
   * refuses. That is the layer whose instruction was wrong (D-257).
   */
...
describe("POST /api/flows/account-opening refuses an undeclared registration at the boundary", () => {
  ...
  /**
   * A DEPLOYMENT DEFECT IS CLASSIFIED BY ITS CAUSE (D-257), not by which layer
   ...
   *
   * And the configured field id stays OFF THE WIRE (D-258): it reaches the
  ...
  /**
   * THE EDITED-RESUBMIT PATH IS REACHABLE AND IT CLEARS (D-254).
   *
  ...
  /**
   * A DEPLOYMENT AN OPERATOR CAN REPAIR IS NOT A DEAD END (D-255/D-256).
   *
  ...
  /**
   * THE SAME CAUSE ANSWERS THE SAME WAY ONE LAYER IN (D-257).
   *
  ...
  /**
   * THE REFERENCE SURVIVES THE WHOLE WAY TO THE BODY (D-260).
   *
