...
const NON_PII_ESCAPES: Array<{ ref: string; why: string }> = [
  ...
  // PROMPT 10 (ADR-0056) domain-configuration vocabulary. The PII field rule
  ...
  // no household data at all. Every entry is exact-match and proven load-bearing
  // by the escapes-suppress-something assertion below.
  { ref: "src/domain/config/bind.ts :: BoundDomainConfig.config.bindings.primitive.evidenceKindParameters", why: "prompt 8's declaration of WHICH PARAMETER NAMES of a primitive carry evidence kinds - parameter names, never evidence" },
  { ref: "src/domain/config/bind.ts :: BoundDomainConfig.config.document.evidence", why: "the domain's evidence REQUIREMENTS section: kinds, declared paths, freshness floors and who may supply them; a configuration never holds an observation" },
  { ref: "src/domain/config/bind.ts :: BoundDomainConfig.config.intents.intent.requiresEvidence", why: "branded EvidenceKind taxonomy ids naming WHICH KINDS an intent needs - a requirement, never evidence contents" },
  { ref: "src/domain/config/bind.ts :: BoundDomainConfig.evidenceSupplierRoles", why: "branded RoleRefs naming who may supply each evidence kind - authority references, never evidence" },
  { ref: "src/domain/config/bind.ts :: FirmRegistry.evidenceSources", why: "a firm's map from evidence-source CLASS to its own source identifier - pointers to systems, never their contents" },
  { ref: "src/domain/config/bind.ts :: RequiredFirmClasses.evidenceSources", why: "the evidence-source CLASS names a document references, derived from the document so a firm knows what to supply - the question FirmRegistry.evidenceSources answers, never an observation" },
  { ref: "src/domain/config/load-closure.ts :: ClosureWorld.document.evidence", why: "the domain's evidence REQUIREMENTS section: kinds, declared paths, freshness floors and who may supply them; a configuration never holds an observation" },
  { ref: "src/domain/config/load.ts :: DomainConfigEnvironment.primitives.evidenceKindParameters", why: "prompt 8's declaration of WHICH PARAMETER NAMES of a primitive carry evidence kinds - parameter names, never evidence" },
  { ref: "src/domain/config/load.ts :: LoadedBinding.primitive.evidenceKindParameters", why: "prompt 8's declaration of WHICH PARAMETER NAMES of a primitive carry evidence kinds - parameter names, never evidence" },
  { ref: "src/domain/config/load.ts :: LoadedDomainConfig.bindings.primitive.evidenceKindParameters", why: "prompt 8's declaration of WHICH PARAMETER NAMES of a primitive carry evidence kinds - parameter names, never evidence" },
  { ref: "src/domain/config/load.ts :: LoadedDomainConfig.document.evidence", why: "the domain's evidence REQUIREMENTS section: kinds, declared paths, freshness floors and who may supply them; a configuration never holds an observation" },
  { ref: "src/domain/config/load.ts :: LoadedDomainConfig.intents.intent.requiresEvidence", why: "branded EvidenceKind taxonomy ids naming WHICH KINDS an intent needs - a requirement, never evidence contents" },
  { ref: "src/domain/config/load.ts :: LoadedIntent.intent.requiresEvidence", why: "branded EvidenceKind taxonomy ids naming WHICH KINDS an intent needs - a requirement, never evidence contents" },
  ...
  { ref: "src/domain/config/load.ts :: LoadedDomainConfig.document.intents.requiresEvidence", why: "branded EvidenceKind taxonomy ids naming WHICH KINDS an intent needs - a requirement, never evidence contents; a domain configuration holds no observation by construction (prompt 10, ADR-0056)" },
  ...
  { ref: "src/domain/config/bind.ts :: BoundDomainConfig.config.document.blockers.resolvingEvidence", why: "EvidenceKind ids naming what would unblock a decision, reached as a nested path through the bound configuration" },
  { ref: "src/domain/config/bind.ts :: BoundDomainConfig.config.document.presentation.copy.evidenceKinds", why: "evidence-kind LABELS, reached as a nested path through the bound configuration" },
  { ref: "src/domain/config/load-closure.ts :: ClosureWorld.document.intents.requiresEvidence", why: "branded EvidenceKind taxonomy ids, reached by the load-time closure checker" },
  { ref: "src/domain/config/load-closure.ts :: ClosureWorld.document.blockers.resolvingEvidence", why: "EvidenceKind ids naming what would unblock a decision, reached by the load-time closure checker" },
  { ref: "src/domain/config/load-closure.ts :: ClosureWorld.document.presentation.copy.evidenceKinds", why: "evidence-kind LABELS, reached by the load-time closure checker" },
  { ref: "src/domain/config/labels.ts :: DomainLabels.evidenceKinds", why: "the evidence-kind LABEL map a surface renders - kind id to human label, never evidence" },
  { ref: "src/infrastructure/config/domain-config-source.ts :: SourcedDomainConfig.config.document.intents.requiresEvidence", why: "branded EvidenceKind taxonomy ids, reached through the published configuration this adapter reads from disk" },
  { ref: "src/infrastructure/config/domain-config-source.ts :: SourcedDomainConfig.config.document.blockers.resolvingEvidence", why: "EvidenceKind ids naming what would unblock a decision, reached through the published configuration" },
  { ref: "src/infrastructure/config/domain-config-source.ts :: SourcedDomainConfig.config.document.presentation.copy.evidenceKinds", why: "evidence-kind LABELS, reached through the published configuration" },
