...
/**
 * DOMAIN-CONFIGURATION FENCE (v3 prompt 10, ADR-0058; the PINNED activation
 ...
 *    enumerated allow-list justified in ADR-0058. An anti-vacuity companion
 ...
 *    a typed value on an id the document does not declare (D-267). Left unfenced,
 ...
 *    at the mint (D-257). Unmarked, it reads green everywhere and then tells an
 ...
 *    two halves of the D-258 channel, structurally: a `correlationId` in its own
 ...
 *    have read the `null`-answering reader D-265 added as a STATED category.
 *
 *  RULE K - NO DEPLOYMENT INTERNAL REACHES A USER-FACING SURFACE OR THE WIRE
 *    (D-259). The screen that told an advisor to restore a YAML path survived
 *    D-256 only because it was a STATIC LITERAL rather than a generated message,
 ...
 *    wrong diagnosis, which this branch refuses everywhere else (D-268).
 *
 ...
 * ADR-0058. These are shipped, externally-visible names the ruling explicitly
 ...
 * `operatorRecoverable` at the mint (D-257). Listing INCLUSIONS is what let
...
export function unmarkedConfigurationRefusals(
  ...
  for (const sourceFile of project.getSourceFiles()) {
    ...
    for (const mint of callsNamed(sourceFile, "appError")) {
      ...
      // reference joining to nothing. Either half missing is the channel D-258
 ...
 * inventing a fallback it can never send (D-265). Both readers state ONE rule -
...
/**
 * THE OTHER CONTAINER KIND, AT THE SAME CEILING (D-269).
 *
...
describe("domain-configuration fence (v3 invariant 3, prompt 10)", () => {
  ...
  describe("detects (companion): incomplete or dishonest work CANNOT pass", () => {
    ...
    it("(J) catches an instruction STATED in a file NO registry lists", () => {
      ...
      // invent an unsendable fallback (D-265).
    ...
    it("(M) catches a truncating constructor that reports its location as an exact one", () => {
      ...
      // there is no limit argument to supply one (D-269).
    ...
    it("catches a module other than the config source naming the configuration directory", () => {
      ...
      // a path, and naming one crosses the same trust boundary D-256 closed.
      expect(configDirectoryReaders(inMemoryProject({
