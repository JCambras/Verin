...
describe("config-hygiene fence (no secret fallback / no live org domain / placeholder .env)", () => {
  ...
  describe("detects (companion): planted violations are caught", () => {
    ...
    it("catches reflected createRequire before it can expose revealSecret", () => {
      const project = inMemoryProject({
        "/src/contracts/secret.ts": `export class SecretValue {}; export function revealSecret(value: SecretValue): string { return ""; }`,
        "/src/app/evil.ts": `
    ...
    it("catches createRequire through Reflect.get.call", () => {
      const project = inMemoryProject({
        "/src/contracts/secret.ts": `export class SecretValue {}; export function revealSecret(value: SecretValue): string { return ""; }`,
        "/src/app/evil.ts": `
          import * as nodeModule from "node:module";
          const created = Reflect.get.call(Reflect, nodeModule, "createRequire")(import.meta.url);
          created("../contracts/secret");
        `,
      });
      expect(detectUnsanctionedReveal(project).some((hit) =>
