...
// with its three-valued verdict IN PUBLIC - MATCHED, DIFFERS carrying the product-owner ruling that
...
// no smoothing, and the DIFFERS rows stay visible until the product owner's signature sitting resolves
...
type ConformanceFile = {
  ...
  cases: { caseId: string; disposition: string; typedQuantityCount: number; verdicts: Verdict[] }[];
};
...
export default async function Conformance() {
  ...
  return (
    <section className="stack" data-testid="verin-conformance-loaded" aria-labelledby="conformance-heading">
      ...
      <p className="meta">
        The decision engine re-derives every case from evidence and configuration alone, and every binding field is compared against the product-owner-signed expectation at oracle head{" "}
        {file.oracleHead.slice(0, 12)}. Three verdicts only: matched; differs, which is never absorbed - each carries the product-owner ruling that dispositions it; or not yet producible, naming the
        prompt that lands the field. Totals: {file.totals.MATCHED} matched · {file.totals.DIFFERS} differ under {file.ledgerEntries} reconciliation-ledger rulings · {file.totals["NOT-YET-PRODUCIBLE"]}{" "}
        not yet producible. This register renders the committed conformance file, regenerated and byte-compared on every build.
      </p>
      {file.cases.map((gc) => (
        <section key={gc.caseId} className="stack" aria-labelledby={`case-${gc.caseId}`}>
          ...
          <p className="meta">{gc.typedQuantityCount} load-bearing quantities read directly from signed typed rows. Summary prose is not an input.</p>
      ...
      <p className="meta">
        A differing field is a recorded fact about this build, not a defect ticket. The four remaining differences are CD-4e reservation orderings retained under their recorded ruling. CD-4b, CD-4c
        and CD-4d converged in the product owner's signing act, never by editing signed bytes here.
      </p>
    </section>
  );
}
