...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  if (principal.ok) {
    ...
    await auditEvent(db, { tenant: tenantOf(principal.value), actor: principal.value.userId, action: "session.revoke", entityType: "Session", entityId: principal.value.sessionId, detail: "Signed out" });
  }
