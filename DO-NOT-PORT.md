...
| 2 | **Dead `getAIAdapter` abstraction** — a provider-switch factory scaffolded but never called | `src/lib/ai/factory.ts:16-41`; `src/lib/ai/port.ts`; `src/lib/ai/adapters/*` | Built-but-not-shipped waste; a port with no caller | **knip dead-export gate** (charter #5); no AI scaffolding until a real caller exists — the sole exception is a ratified-spec boundary proven by a fence in the same PR (ADR-0031), which a `getAIAdapter`-style provider-switch never is |
