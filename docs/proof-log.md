...
- M-C (arm 20): the reserve arithmetic stubbed to an effective liquidity of zero TYPECHECKS and the live-proceed battery fails on 'Henderson at 75000 must proceed, got blocked' - a PASS-emitting check that survives a stubbed implementation is not a check.
- Rule-removal counterexamples: the material-conflict rule deleted fails the Delgado battery (arm 21, the blocker set no longer matches); the regulatory-precedence rule deleted fails the Ashford battery on 'must prohibit, got proceed' (arm 22).
