...
- The den.v1 identity cannot drift (arm 40): one nibble changed in the committed engine-identity.json fails the in-suite equality AND the registry's regenerate-and-byte-compare jaw (REGEN-CHECK reproduced: false), the control passing first. [RECONCILED by the PR-5b ruling: this entry originally wrote 'the control passing 55/55 first', but the exact control command runs src/tools/e16.test.ts alone and its transcript prints 50/50; 55 is the WHOLE suite's count, which belongs only to the green-run line below. The transcripts were always right - the prose overstated them.]
...
## PR-5c-i proofs (prompt 5 sections 5C/6; raw transcripts: docs/evidence/pr5c1-transcripts.tar, sha256 1e9659a0...; battery head dd5c175d - the PR head differs only by this evidence commit). Every mutation transcript opens with the CLEAN-TREE CONTROL of its exact check command PASSING before the mutation lands (the standing rule), and every suite check runs the FULL test file - the e16 file alone counts 54 tests at this head; the whole suite counts 59, cited only for the green run.
- M-B, signed bytes: one byte of GC-01's fixture flipped in the scratch clone at the blob object itself (git replace; the pin registry untouched). The controls pass first (the generator regenerates byte-identically; the full file 54/54); then the hash check fires BEFORE parsing, prints both digests (pinned 9144263d..., read e29f2e46...), the generator and the suite both refuse, and the revert is proven byte-exact against the pin.
- M-D, direction one: the ordering rule "fixed" so the four CD-4e differences vanish - the suite fails until the ledger entries are deleted, naming every one ("stale ledger entry GC-01/GC-10/GC-12/GC-13...:ledger.reservationOrdering ... it cannot rot into an excuse; delete it").
- M-D, direction two: a listed entry re-added for a field that matches (GC-02:disposition) - the suite fails again, naming it stale. Neither direction can be absorbed.
- M-E, vacuity: the runner pointed at ZERO cases fails ("refuses zero cases: a run that grades nothing must never report clean"), and a case stripped to all NOT-YET-PRODUCIBLE verdicts fails ("an all-deferred case is not graded") - it cannot count as graded.
- M-F, determinism: the sixteen graded under America/New_York and Asia/Tokyo - the conformance file and the 18-row replay manifest are byte-identical across zones and equal to the committed bytes.
- M-J, the grammar regression, is a COMMITTED test, not a battery arm: every produced key conforms to the CD-4d grammar and exactly GC-10 deviates on the signed side, the named pre-signature divergence; after the pin moves the exceptions list must empty for it to keep passing.
- The carried PR-5b ruling inclusion, discharged here: the PR-5b arm-40 entry's control-count phrase is reconciled in place above (the exact control command prints 50/50 at its head; 55 was the whole suite's count).
- The restored green run (green-restored.txt): 59 unit tests; both generated artifacts regenerate byte-identically.

## PR-5c-ii proofs (prompt 5 section 5C steps 5-6; raw transcripts: docs/evidence/pr5cii-transcripts.tar, sha256 70319131...; battery head bb5cc697 - the PR head differs only by this evidence commit). Every mutation transcript opens with the CLEAN-TREE CONTROL of its exact full-file check command PASSING (54/54 for the e16 file at this head) before the mutation lands.
