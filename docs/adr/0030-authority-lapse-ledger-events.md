# ADR-0030: Authority-lapse ledger events extend the v3 `LedgerEntry` union until prompt 7 lands it

**Status:** Accepted (deferral with trigger)
**Date:** 2026-07-28
**Deciders:** Founding architect (recording the captain's D-061 signature on GC-16), captain (D-061)
**Relates to:** D-061 (signed money truth, evidence completeness, canonical status planes); ADR-0023 (v3 adoption - deviations from v3 are recorded by ADR); ADR-0026 (the existing v3-deviation register); charter #1 (fence every invariant), #4 (detection is not verification)
...
D-061 signed GC-16 (`GC-16-specialist-review-expiration`) as product truth: a specialist-review
...
| Leave it as a code comment plus the D-061 journal entry | The charter requires an ADR for v3 deviations, and a comment cannot carry the collapse trigger or be enforced - which is exactly how the "must be a v3 LedgerEntry type" message went stale. |
