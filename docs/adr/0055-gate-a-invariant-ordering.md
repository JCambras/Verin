...
Constructor calls and constructor parameters participate in that same invocation index. Destructured
intrinsic aliases resolve through their stable source-object property, and callable values obtained
through `Object.getOwnPropertyDescriptor`, `Object.getOwnPropertyDescriptors`, or
`Reflect.getOwnPropertyDescriptor` retain the original receiver and property provenance. An incomplete
parameter resolution is unsafe even when it yields no known path.
...
blocking.
The blocking workflow trigger accepts only the complete unfiltered `push` and `pull_request` event set,
in mapping or array form. Extra, non-string, duplicate, filtered, or unsupported event declarations
invalidate all command evidence rather than relying on a partial trigger parse.
...
- Every `ci-gate`, in a gate requirement and in an invariant mechanism alike, names the `command` its
  ...
  shell-command parse. Every supported workflow, job, container, and step field must also satisfy its
  complete value schema across the workflow before any command is evidence. The workflow must carry
  unfiltered normal `push` and `pull_request` triggers,
...
- `parseCiJobs` is the repo's one structured CI authority, read by three call sites - the gate
  ...
  provenance, including stable global-intrinsic aliases, computed intrinsic members, and local
  higher-order parameter flow, so neither a neutralizer nor a registered hook can hide behind
  reflective dispatch. `Object.assign` mutation through a stable global `Object` alias is included.
...
- Reflective callable resolution composes direct or bound `Reflect.apply`, `Function.call`, and
  ...
  computed global-intrinsic members, locally invoked callable parameters, and bound callables invoked
  through `call` or `apply`. Incomplete resolution is non-evidence. Gate 0
...
### Amendment (2026-08-10, consolidated wrap-up ruling `g8-relight-askuser`): rule-set decomposition and measured tooling envelope

- The shared rule set decomposed from one 2,299-line `scripts/v3-gates.lib.ts` into per-seam modules
  under `scripts/v3-gates/` (model, shell, ci-values, ci-schema, ci-evidence, ci-workflow,
  invariant-ratchets, gate-ratchets, ordering, registry), every module inside the 500-line per-file
  ceiling, all reached through the unchanged `scripts/v3-gates.lib.ts` entry that BOTH the
  gate-ordering fence and the blocking runner import. No rule changed in the split.
- Registry-structural validation (`validateRegistry`) moved INTO the shared set
  (`scripts/v3-gates/registry.ts`): the v3-invariants fence and the blocking runner now consume one
  implementation, closing the divergent re-implementation the one-shared-rule-set mandate exists to
  prevent. The runner fails closed on it before printing any state, and the fence proves that guard.
- The shipped-activation mechanism ratchet extended monotonically to the registry's activation facts:
  invariant 16 (prompt 9) with its complete tuple set, invariant 2's fourth mechanism, and invariant 5's
  five-tuple set, in registry order.
- The corpus CI job invokes its owned entry points directly (`pnpm exec tsx scripts/corpus-validate.ts`,
  `pnpm exec tsx scripts/corpus-report.ts`), and `charter-map.json` pins the exact validate command on
  its `ci-gate` mechanism, per the name-only-mappings-are-invalid rule above.
- MEASURED TOOLING ENVELOPE (amends the ADR-0052/D-177 figure the line-budget fence records):
  `scripts/**` re-measured with the line-budget fence's own algorithm on the tree as this round lands is
  **12,139 lines** - the gate-constitution modules above, the corpus and demo entry points, and the
  decomposition's module headers account for the growth over D-177's 9,053. The tooling ceiling moves
