...
Every external/house-CRM mutation goes through a single **`auditedWrite`** helper under a runtime-sealed
`WriteActor`, so tenant and actor attribution cannot be supplied as unrelated strings. The helper performs
the op and audits **both** success and failure paths by construction (ADR-0007); latency is recorded by the
