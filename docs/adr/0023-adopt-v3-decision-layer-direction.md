...
(v3 remains binding on current Verin and remains legacy-oracle evidence, but does not select or bind
fourth-generation composition)
...
- `PRODUCT-DIRECTION.md` v2 lands (C7): it must restate the product story under this framing or name the
  contradiction.
- Current Verin's Phase 1 completes (all 30 invariants active-pass): fold the lessons into a review of
  what v3 §17 missed, per the known-risks register (v3 §20). That review does not select or authorize
  fourth-generation composition (ADR-0060).
