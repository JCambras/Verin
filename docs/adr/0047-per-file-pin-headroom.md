# ADR-0047: The per-file pin gets the same bounded headroom as the layer ceiling

**Status:** Accepted
**Date:** 2026-08-06
**Deciders:** Founding architect
**Amends:** ADR-0018 and ADR-0046
**Relates to:** Charter non-negotiables #1, #4, #10

## Context

ADR-0046 restored the explanatory comments an earlier correction had compressed out of
...
ADR-0046 itself records why that figure is wrong: the file measured 507 lines before the
...
- The `max-file-size` pin for `src/infrastructure/store/migrations.ts` rises from 520 to
  **560**, against a measured 510 - fifty lines of bounded correction headroom, roughly four
  times the twelve lines of prose ADR-0046 restored. It is sized like the layer amendment:
...
- The reasoning ADR-0046 gives for pinning rather than splitting still holds and is not
...
|-------------|--------------|
| Leave the pin at 520 | Ten lines is the same exhausted headroom ADR-0046 was written to end; the next correction on this file pays in prose or in another ADR. |
| Split `migrations.ts` to stay under the default | Rejected in ADR-0046 for reasons this ADR does not disturb: the natural seam is already extracted. |
...
| Pin far above measurement so no amendment is ever needed | Unbounded headroom stops the ratchet from measuring anything - ADR-0046 rejected the same move at the layer. |
