...
The rule is about **committed** entries, so the tree walk drops an entry only when git itself confirms
it is untracked, and only under an entry name the repository's `.gitignore` already refuses to track
(`.DS_Store` today). An untracked platform dropping is neither committed nor committable, and failing a
blocking gate on one would punish opening the directory in a file browser. The NAME buys nothing on its
own: a force-added dropping is a committed byte no generator emits and no digest binds, so it is
surfaced and refused like any other unaccounted entry. When git cannot answer at all - no repository, no
binary - the walk drops nothing, because an entry no one can prove untracked must still be accounted
...
DST context in the synthetic partition requires at least two cited recent-change records whose exact UTC and local renderings share
