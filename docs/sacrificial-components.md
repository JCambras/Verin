...
| **Minimal labeled seed** (`scripts/db-seed.ts` + `scripts/seed-decision-ledger.ts`, Phase E) | Smallest labeled dataset the skeleton, its Playwright specs, the house-CRM console, the `/app/ledger` register, and the load gate need | The first demo milestone (captain D-005 / ADR-0012) expands seed into the real populated world; the decision-ledger half is superseded when a real producer appends decision events (v3 prompt 7 ships the append boundary unwired as a named deferral, D-116/D-119) | Every row carries `asOf` and visible provenance (charter #3): house-CRM rows `source=verin-crm`, decision-ledger rows `prov_source='fixture'` rendered behind the synthetic-fixture badge; never feeds a compliance decision (`canFeedComplianceDecision` refuses it); the seed REFUSES APP_ENV=production (publicly committed demo credential) |
