...
| CC8.1 | Change management | protected main, PR template, CODEOWNERS; published domain configuration is versioned, hash-pinned (`config/domains/versions.json`) and carries authorship provenance | CI gates block merge; `domain-configuration` (a published version's bytes are immutable; a declared change record is checked against the computed diff) | this doc; `.github/`; ADR-0058, `docs/domain-config.md` | PR/merge history; no-mistakes runs |
