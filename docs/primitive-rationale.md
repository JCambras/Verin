...
- **Binding multiplicity is bounded by published-key scope** (captain rulings
  ...
  `fu-binding-multiplicity-check`, owned by prompt 16 (D-264).** Prompt 10's config load
...
- **Absent evidence is the validation stage's problem, not the screen's** (captain ruling
  ...
  `fu-restriction-evidence-required`, owned by prompt 15 (D-264).** Prompt 10 was to
