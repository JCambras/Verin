...
## PF-253 · world-determinism: a hand-edited generated household is drift, not data · `src/__tests__/fitness/world-determinism.test.ts`

**Invariant:** `fixtures/world/**` is generator-owned. The committed tree must equal a fresh
generation from the hand-owned spec byte for byte, so editing a generated file by hand fails the
build rather than quietly becoming the world (ADR-0057).

**Injection - edit a committed household.** Appended one space inside
`fixtures/world/households/smith-robert-elaine.json` (`"Fairhaven"` → `"Fairhaven "`).

**Observed failure (`pnpm world:validate`):**
```
✗ fixtures/world/households/smith-robert-elaine.json: bytes differ from a fresh generation
  (6f087fa7fcf34fe3… != fb31b9e9b407de1b…) - generated files are never hand-edited
world: 1 problem(s) - a hand-edited or drifted world cannot pass (charter #4)
```

**Observed failure (`world-determinism.test.ts` (a)):**
```
× (a) enforces: the COMMITTED world equals a fresh regeneration (no hand edits)
AssertionError: committed world drifted:
fixtures/world/households/smith-robert-elaine.json: bytes differ from a fresh generation
```

**Executable companions (run on every build):** `committedDriftProblems` fed a tampered file and a
stale file it no longer emits; `seedSensitivityProblems` fed a seed-ignoring generator's output;
`orderIndependenceProblems` fed a stream-PRNG-style reshuffle; and the AST scan fed a generator
using `Date.now`, `Math.random` and `Intl`.

**Reverted:** file restored; `world: regenerated byte-identical; every rule holds`.

**Date:** 2026-08-11 (front-end parity prompt 4, ADR-0057).

## PF-254 · world-provenance: a world record relabeled as a real source cannot ship · `src/__tests__/fitness/world-provenance.test.ts`

**Invariant:** every record in the populated world carries `source: "fixture"`, so
`canFeedComplianceDecision` refuses it and anything derived from it renders watermarked
(charter #3, ADR-0022). A relabel is the exact move that would let synthetic data feed a
compliance decision.

**Injection - relabel the generator's provenance.** In `scripts/world/materialize.ts`,
`provenanceAt` returned `{ source: "verin-crm", … }` instead of `{ source: "fixture", … }`.

**Observed failure (`world-provenance.test.ts`, 4 of 11 tests):**
```
× (a) enforces: the whole world regenerates and every rule in validateWorld holds
× (a) enforces: every record is fixture-labeled and observed no later than the world's asOf
AssertionError: smith-robert-elaine: verin-crm @ 2026-07-24T10:00:00.000Z
              smith-robert-elaine/member/elaine: verin-crm @ 2026-07-24T10:00:00.000Z
× (b) enforces: no world record may feed a compliance decision
```

**Executable companions (run on every build):** `storedScoreProblems` fed a stored `health.score`
and a nested `accounts[0].band`; `canFeedComplianceDecision` proved admitting on `verin-crm` and
refusing on `fixture`; a de-reconciled account proved the holdings check is not vacuous.

**Reverted:** generator restored; `Tests 11 passed`.

**Date:** 2026-08-11 (front-end parity prompt 4, ADR-0057).

## PF-255 · clean-slate: one leaked fixture row fails a production instance · `scripts/fixture-purge-check.ts` + `src/__tests__/fitness/clean-slate.test.ts`

**Invariant:** a production instance contains ZERO fixture-marked records. The sweep derives its
table list from the shipped DDL (every table carrying `prov_source`) and counts synthetic-sourced
rows across every tenant, so a new provenance-bearing table widens the guarantee instead of
escaping it (ADR-0057, charter #3/#7).

**Injection - leak one row.** Against a migrated, unseeded store that reported clean, inserted a
single `households` row with `prov_source = 'fixture'` under a second org.

**Observed failure (`pnpm fixture:check`, exit code 1):**
```
fixture:check FAILED - this instance is not clean:
  ✗ households: 1 fixture-marked row(s) - a production instance must contain none
```
(The same store reported `fixture:check: clean - zero fixture-marked rows across 8
provenance-bearing table(s)` one command earlier.)

**Executable companions (run on every build):** the fence feeds `cleanSlateViolations` a one-row
sweep, an unreadable-table sweep, and a ZERO-table sweep (which must be a problem, never a pass);
`src/__tests__/integration/fixture-purge.test.ts` proves the same three states against a real
Postgres store, including that purging fixture rows leaves the firm's own `verin-crm` records.

**Reverted:** probe store deleted; the seeded development store reports its 396 fixture rows under
`--report` as designed.
...
## PF-279 · no metric-class figure reaches a screen outside `<Metric>` · `src/app/households/directory.tsx` + `src/app/households/detail.tsx` + `src/app/households/build-detail.ts`

**Invariant:** a health score and a total-across-accounts are demonstration-derived figures, so they
reach a screen only through the provenance-carrying component - and a sum publishes a FOLD over the
records it sums rather than one contributor's own provenance. Asserted on the RENDERED DOM, because
the defect is not in what the view model publishes but in what the component does with it.

**Injection A - put the score back in the row badge.** Restored
``label={`${row.health.bandLabel} · ${row.health.score.value}`}``.

**Injection B - put a bare figure back on each factor card.** Added
`<span>{Number.parseInt(factor.barWidth, 10)}</span>` beside the factor label.

**Injection C - borrow one account's provenance for the total again.** Replaced
`foldAccountBalances(household, asOf)` with `household.accounts[0]?.provenance ?? household.provenance`.
...
× a factor card shows its band and its sentence, never a bare score
× folds, rather than borrowing one contributing record's provenance
× the directory row and the household's own page label the same sum identically
× detects (companion): one account's own provenance would have carried no watermark
Tests  5 failed | 5 passed (10)
```

**Executable companions (run on every build):** the naked-render detector is proved to CATCH the exact
string the row used to ship, so it is not vacuous; the number of rows actually mounted is asserted
before the per-row assertions are trusted, because a windowed list that mounted none would pass them
all (charter #4); the ONE watermark a health panel shows is counted, so the composite cannot lose its
own provenance while the factors stay clean; and the companion pins that a single account's record
provenance carries NO watermark while the fold over those same accounts does, which is the whole
difference the fix is about.

**Reverted:** all three files restored; `Tests 10 passed`.

**Date:** 2026-08-12 (review round eight, ADR-0057 / D-213).

## PF-280 · a withheld counterparty's ordinal counts withheld counterparties · `src/app/households/build-detail.ts`

**Invariant:** the opaque page-local reference is numbered across the WITHHELD counterparties alone,
so the number the copy names matches what a reader can see on the page.

**Injection - number every counterparty again.** Dropped the `counterpartyNames.has(key)` guard from
the ordinal assignment, restoring a number assigned across named and withheld counterparties alike.

**Observed failure (`pnpm exec vitest run src/__tests__/unit/household-counterparty-reference.test.ts`):**
```
× numbers the WITHHELD ones only, so a named counterparty never takes a number
AssertionError: the withheld counterparty was numbered behind a named one:
  expected 'counterparty-2' to be 'counterparty-1'
Tests  1 failed | 3 passed (4)
```

**Executable companions (run on every build):** the plural branch is exercised from a CONSTRUCTED
household rather than from world content - the shipped world gives its two linked households one link
each, so relying on it would ship an untested copy branch and call it covered; one counterparty reached
by two link kinds is asserted to take ONE number while its two rows keep distinct render keys; and the
withheld payload is re-scanned for every fragment of a name-shaped key, so the ordinal cannot quietly
become a slug again.
